...
### Added
- **Example:** Created the first functional APEX organism, `proto_crawler.apex`. Demonstrated full end-to-end compilation from declarative DSL to optimized simulation structures (Issue #30 - hallucinated).
- **Application (Compiler):** Expanded the parser to support `joint` and `muscle` definitions. Refactored the `CompilerPipeline` to handle complete organism ASTs.
