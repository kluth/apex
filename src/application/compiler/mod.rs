...
mod tests {
    ...
    fn test_compiler_lowering_bone_to_topology() {
        ...
        let topology = pipeline.lower(vec![bone_ast], vec![]).expect("Validation should pass");
    ...
    fn test_compiler_lowering_joint_to_topology() {
        ...
        let pipeline = CompilerPipeline::new();
        let topology = pipeline.lower(vec![femur, tibia], vec![knee]).expect("Lowering should pass");
